...
# The VPS daily-sync produces these once ~/.second-brain/backup.key is provisioned
# there; until then the remote glob matches nothing and we log a benign "none yet".
...
if rsync $RSYNC_OPTS "$VPS:~/$REMOTE_DATA/backups/offsite/brain-*.db.zst.enc" "$OFFSITE_LOCAL/" 2>> "$LOG_FILE"; then
  ...
  log "offsite snapshots: none to pull yet (backup key unprovisioned on VPS?)"
fi
